...
class Post:
    def __init__(self, _id, _post_type_id, _creation_date, _score, _view_count, _body, _owner_user_id,
        ...
        self.creationDate = datetime.strptime(_creation_date[:-4], "%Y-%m-%dT%H:%M:%S") if _creation_date is not None else None
        self.score = int(_score)
        self.viewCount = int(_view_count) if _view_count is not None else 0
        self.body = _body
        self.ownerUserId = int(_owner_user_id) if _owner_user_id is not None else None
        self.lastActivityDate = datetime.strptime(_last_activity_date[:-4], "%Y-%m-%dT%H:%M:%S") if _last_activity_date is not None else None
        self.title = _title
        self.tags = _tags
